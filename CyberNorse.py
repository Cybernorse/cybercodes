...
def Cyber_norse_bot():              #extracting the websites data for links and saving them into the file 
    ...
    global cyber_file
    global cyber_url
    cyber_url="https://en.wikipedia.org/wiki/Dark web"
...
def botnet_webmining_data():
      ...
      #  for para in cyber_link2.find_all('p',text=True,recursive=True):
        ...
        diff_seq=difflib.SequenceMatcher(autojunk=True)
        ...
        if seq_ratio >= 0.6 :
            ...
            ML_info=str_cyber_title + ' ' + str(seq_ratio)
            
            csv_file=open("/root/bot_data_testing/botnet_webmining_data.csv","w")
            csv_text=csv.writer(csv_file,dialect='excel')
            csv_text.writerow(re.sub(r"(\w+\n)\1+",r"\1",str(ML_info)))
            csv_file.close()
