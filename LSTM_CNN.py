...
from tensorflow.keras.layers import LSTM,Convolution1D,Lambda,MaxPooling1D
...
class lstm:
    def __init__(self):
        ...
        regressor.add(Convolution1D(filters=50 ,kernel_size=1 , activation = 'tanh'))
        # regressor.add(Lambda(lambda x: tf.expand_dims(regressor.output, axis=-1)))
