...
class parsing:
    def __init__(self,*args,**kwargs):
        ...
        slicers=[]
        self.pp=[]
        for i,z in enumerate(self.posts):
        ...
        for i in range(len(slicers)-1):
            g_arr=[s for s in self.posts[slicers[i]:slicers[i+1]]]
            g=g_arr[0].split(": ")
            g_arr.insert(1,g[-1])
            if len(g_arr)>1:
    ...
    def log_info(self):
        ...
        adde=[]
        lefts=[]
        # getting phone numbers for addedby, added and left
        for i in self.logs:
            if "added".lower() in i.lower():
                addedby=re.findall(r'-(.*[+1]\s?[(]\d{3}[)]?\s?\d{3}-\d{4}\s)\s?added',i)
                if addedby:
                    addy.append(addedby[0].strip(' '))
                added=re.findall(r'-\s?\s?was\s?added|added\s?(.*)',i)
                if added:
                    for i in added[0].split(','):
                        for x in i.split('and'):
                            adde.append(x)
            if "left".lower() in i.lower():
                left=re.findall(r'-(.*[+1]\s?[(]\d{3}[)]?\s?\d{3}-\d{4}\s)',i)
                if left:
                    lefts.append(left[0].strip(' '))

        # removing the duplicates from the lists
        uni_addy=[]
        for i in addy:
            if i not in uni_addy:
                uni_addy.append(i)
        uni_add=[]
        for i in adde:
            if i not in uni_add:
        ...
        uni_left=[]
        for i in lefts:
            if i not in uni_left:
                uni_left.append(i)
        
        # getting post timestamps
        app=[]
        for i in self.pp:
            pps=re.findall(r'-(.*[+1]\s?[(]\d{3}[)]?\s?\d{3}-\d{4}\s?)',self.posts[i])
            if pps:
                app.append(pps[0].strip(' '))

        # getting the number of posts for each phone number and writing to csv file. 
        counter=[]
        uniq_cumu=uni_addy+uni_add+uni_left
        for i in uniq_cumu:
            counter.append(app.count(i))
        data=[addy,adde,lefts,uniq_cumu,counter]
        zipped=zip_longest(*data,fillvalue='')
        with open("/home/bigpenguin/predictions and datasets/datasets/timestamps.csv","w") as time_csv:
            csvWriter = csv.writer(time_csv,dialect='excel')
            csvWriter.writerow(['Addedby','Added','left','cumulative list','posts'])
